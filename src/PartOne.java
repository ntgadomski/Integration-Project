...
public class PartOne {
	...
	public static void main(String[] args) {
		...
		System.out
				.println(" byte = -128 to 127 inclusive short = -32768 to 32767 \n int = -2^31 to 2^31-1 \n long = -2^63 to 2^63-1 "
						+ "\n float & double = #.##  \n boolean = true || false \n char = ASCII");

		int myAge = 23; // equal operator means is assigned to
		double myHeight = 5.6;
		String myName = "Nickolas Gadomski";
		final String DATE_OF_BIRTH = "04/05/1995"; // final means constant
													// (never changes)

		String yearOfBirth = DATE_OF_BIRTH.substring(6, 10);
		System.out.println("I was born in the year " + yearOfBirth);

		System.out.printf("My name is %s, I was born on %s, "
				+ "so I'm %d years old, and my" + " height is %f", myName,
				DATE_OF_BIRTH, myAge, myHeight); // formating with escape
													// sequences

		System.out.println("-");

		double sum = (double) (myAge) + myHeight; // casting you turn a lighter
													// type to a heavier type
		...
		System.out.println("chatAt(2) " + myName.charAt(2)); // return a char =
																// s
		System.out.println("substring(2, 7) " + myName.substring(2, 7)); // returns
		System.out.println("length() " + myName.length()); // returns length of
															// such string

		String result; // the ternary operator will assign a value
		System.out
				.println(result = (nickMale == false) ? "Nick is indeed a male! (ternary)" // conditional
																							// operator
																							// '?'
		...
		System.out.println(charlie.equals(alpha)); // .equals() means String
													// content
		...
		System.out.println("alpha.compareTo( charlie ) -> "
				+ zeroIsLexicographicallyEqual);
		...
		int number = -34;
		if (number > 0 || number < 0) { // conditional operator with OR and
										// relational operators < , >
			System.out.println("Thank God number is not 0.");
		}
		number = 30;
		if (number < 50 && number > 25) // conditional operator with AND and
										// relational operators < , >
		...
		System.out.println("number += 2 =" + (number += 2)); // increment
																// variable by n
																// (n=2 in this
																// case)
	...
	private static String myGuyMethod(String name) { // method head with
														// parameters
